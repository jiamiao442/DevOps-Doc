...
containerd-1.7.27 + k8s-1.33.3（最新）（kubeadm方式）（containerd容器运行时版）

> kubeadm方式安装最新版k8s-1.33.3（containerd容器运行时）
>
> containerd-1.7.27 + k8s-1.33.3（最新）（kubeadm方式）

> containerd-1.7.27
>
> k8s-1.33.3

> - k8s-master1（rockylinux-9.6）（4c8g-200g）
> - k8s-master2（rockylinux-9.6）（4c8g-200g）
> - k8s-master3（rockylinux-9.6）（4c8g-200g）
> - k8s-node1（rockylinux-9.6）（8c16g-200g）
> - k8s-node2（rockylinux-9.6）（8c16g-200g）
> - k8s-node3（rockylinux-9.6）（8c16g-200g）
...
### 1、安装containerd-1.7.27（官方源、腾讯源）(rockylinux-9.6 )
...
### 2、安装k8s（kubeadm-1.33.3、kubelet-1.33.3、kubectl-1.33.3）（官方源）(rockylinux-9.6 )
...
yum -y install kubeadm-1.33.3 kubelet-1.33.3 kubectl-1.33.3
...
> - **结论**：VIP 和节点在同一子网内，可以使用 `kube-vip` 的 ARP 模式。

> ### **场景 2：不同子网（需 BGP 模式）**
>
> - 集群节点：
>   - Node1: `192.168.1.10/24`
>   - Node2: `192.168.2.10/24`
> - VIP: `192.168.1.100/24`
> - **结论**：VIP 和 Node2 不在同一子网内，ARP 模式无法工作，必须使用 BGP 模式。
...
kube-vip manifest pod \
    ...
    --bgp \
    --localAS 65000 \
    --bgpRouterID 192.168.1.11 \
    --bgppeers 192.168.0.12:65000::false,192.168.0.13:65000::false | tee /etc/kubernetes/manifests/kube-vip.yaml
````
...
kube-vip manifest pod \
    ...
    --bgp \
    --localAS 65000 \
    --bgpRouterID 192.168.1.12 \
    --bgppeers 192.168.1.11:65000::false,192.168.1.13:65000::false | tee /etc/kubernetes/manifests/kube-vip.yaml
````
...
kubeadm config images list --kubernetes-version=v1.33.3 --image-repository registry.aliyuncs.com/google_containers
```
...
kubeadm config images pull --kubernetes-version=v1.33.3 --image-repository registry.aliyuncs.com/google_containers
```
